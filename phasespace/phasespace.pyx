...
cdef extern from "_phasespace.h":
...
def phase_space_avg(np.ndarray[np.double_t, ndim=1] xin,
...
@cython.boundscheck(False)
def phase_space_count(np.ndarray[np.double_t, ndim=1] xin,
                            np.ndarray[np.double_t, ndim=1] yin,                            
                            np.ndarray[np.double_t, ndim=1] xout,
                            np.ndarray[np.double_t, ndim=1] yout):
    cdef int ndim0 = xin.shape[0]
    ...
    phase_space_avg(&xin[0], &yin[0], &xout[0], &yout[0], ndim0, nx, ny, &phase[0, 0])
